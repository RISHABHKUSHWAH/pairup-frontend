...
import { mentorNotifications } from '../../api/storage/mentorStorage';
...
export default function MentorNotificationsPage() {
  ...
  const loadNotifications = async () => {
    ...
    try {
      ...
      // fallback to mentor local storage if backend offline
    }

    const localData = mentorNotifications.getNotifications();
    if (remote.length > 0) {
  ...
  const handleViewDetail = async (item) => {
    ...
    const targetLink = resolveNotificationLink(item, 'mentor');
    if (targetLink) {
      navigate(targetLink);
    }
  };
  ...
  const getTypeLabel = (type) => {
    switch (type) {
      ...
      case 'proposal':
        return 'Proposal / Pitch';
      case 'session':
        return 'Session';
      case 'message':
        return 'Message';
      case 'payment':
        return 'Earnings / Escrow';
      case 'review':
  ...
  return (
    ...
    >
      <p className="sub" style={{ marginBottom: '20px' }}>
        Track incoming learner problem requests, accepted pitches, session reminders, released payouts, and learner reviews.
      </p>
      ...
      <div className="admin-filter-tabs">
        ...
        <button
          type="button"
          className={`admin-filter-tab ${activeFilter === 'problem' ? 'active' : ''}`}
